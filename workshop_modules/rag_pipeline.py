...
print(response)
        ...
        st.markdown("Upload a document and ask questions about it to see a simplified RAG pipeline in action.")
            ...
            if "doc_processed" in st.session_state and st.session_state.doc_processed:
                ...
                if st.button("Submit Question") and query:
                    with st.spinner("Retrieving relevant information and generating answer..."):
                        ...
                        # Show source documents (simplified)
                        st.subheader("Source Documents")
                        
                        with st.expander("Document Sources (Click to expand)"):
                            st.markdown(f"**Source 1:** {uploaded_file.name}, Chunk 3")
                            st.markdown("""
                            RAG (Retrieval-Augmented Generation) systems combine the power of large language models with external knowledge retrieval. 
                            The main benefits include improved factual accuracy by grounding responses in reliable sources, the ability to access 
                            up-to-date information beyond the model's training cutoff, and significant reduction in hallucination rates...
                            """)
                            
                            st.markdown(f"**Source 2:** {uploaded_file.name}, Chunk 7")
                            st.markdown("""
                            Unlike traditional LLMs that rely solely on parametric knowledge, RAG systems can incorporate domain-specific information
                            without expensive retraining. This makes them ideal for specialized applications such as legal document analysis, 
                            medical research, and technical support where accuracy is critical...
                            """)
